...
function importFile(filePath) {
    logger.debug('Importing file: ' + path.basename(filePath));
    return Promise.invoke(fs.readFile, filePath)
        .then(function (metadata) {
            ...
            logger.debug('Import complete for file: ' + path.basename(filePath));
            return catalog;
        });
}
...
CatalogService.prototype.initializeSchema = function (done) {
    var catalog, self = this, modelPath = path.resolve(__dirname, METADATA_FOLDER);
    ...
    return Promise.invoke(fs.readdir, modelPath)
        .then(function (result) {
            ...
            result.forEach(function (fileName) {
                if (path.extname(fileName) === '.xml') {
                    promises.push(importFile(path.resolve(modelPath, fileName)));
                }
            });
        ...
        .then(function (result) {
            logger.debug('Deleting old catalog');
            catalog = result;
        ...
        .then(function () {
            // create new catalog
            logger.debug('Creating new catalog');
